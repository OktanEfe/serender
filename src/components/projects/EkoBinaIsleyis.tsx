  ...
  return (
    <section className="relative w-full bg-[#F5EFE6] py-24 md:py-36 overflow-hidden">
      ...
      <svg className="absolute bottom-0 left-0 w-[320px] h-[320px] opacity-[0.07] pointer-events-none" viewBox="0 0 320 320" fill="none">
        <path d="M10 310 C10 310 160 260 230 150 C290 55 240 10 240 10 C240 10 225 100 165 190 C90 290 10 310 10 310Z" fill="#1E4D3A"/>
        <path d="M10 260 C10 260 130 215 190 120 C238 44 200 12 200 12 C200 12 190 90 135 175 C68 270 10 260 10 260Z" fill="#1E4D3A"/>
      </svg>

      <svg className="absolute top-0 right-0 w-[260px] h-[260px] opacity-[0.06] pointer-events-none" viewBox="0 0 300 300" fill="none">
        <path d="M290 10 C290 10 160 10 70 110 C0 190 30 285 30 285 C30 285 25 165 115 85 C198 12 290 10 290 10Z" fill="#1E4D3A"/>
        <path d="M260 45 C260 45 150 45 80 135 C20 208 48 268 48 268 C48 268 38 165 115 98 C198 28 260 45 260 45Z" fill="#1E4D3A"/>
      </svg>
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 lg:px-8">
        ...
        <div className="mb-24 md:mb-32">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-4">
            <div>
              <span className="text-[10px] font-medium tracking-[0.25em] text-[#1E4D3A]/60 uppercase mb-4 block">
                Uygulama Aşamaları
              </span>
          ...
          <div className="hidden md:grid grid-cols-5 gap-0">
            {stages.map((stage, i) => (
              <div key={i}>
                <p className="text-[80px] font-extralight text-[#1E4D3A]/20 leading-none mb-4 -mt-2">
                  {stage.num}
                </p>
          ...
          <div className="flex flex-col md:hidden">
            {stages.map((stage, i) => (
              <div key={i} className="flex gap-6 py-6 border-b border-[#1E4D3A]/10 last:border-0">
                <p className="text-[42px] font-extralight text-[#1E4D3A]/25 leading-none flex-shrink-0 w-16">
                  {stage.num}
                </p>
                <div className="pt-1">
        ...
        <div className="flex items-center gap-6 mb-20">
          <div className="flex-1 h-[1px] bg-[#1E4D3A]/15" />
          <span className="text-[10px] font-medium tracking-[0.2em] text-[#1E4D3A]/40 uppercase flex-shrink-0">
            İşleyiş Modeli
          </span>
          <div className="flex-1 h-[1px] bg-[#1E4D3A]/15" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
          ...
          <div className="flex flex-col mt-8 md:mt-0">
            {model.map((item, i) => (
              <div key={i} className="flex items-start justify-between py-5 border-b border-[#1E4D3A]/10 last:border-0">
                <span className="text-[11px] font-semibold tracking-[0.15em] text-[#1E4D3A]/60 uppercase w-32 flex-shrink-0 pt-0.5">
                  {item.label}
                </span>
